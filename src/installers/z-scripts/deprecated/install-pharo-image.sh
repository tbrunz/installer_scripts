...
PHARO_LINUX_URL="https://files.pharo.org/platform"
PHARO_ARM_URL="https://files.pharo.org/vm/pharo-spur32/linux/armv6"

PHARO32_PKG="Pharo${PHARO_VERSION}-linux.zip"
PHARO32_DIR_NAME="pharo${PHARO_VERSION}"

PHARO64_PKG="Pharo${PHARO_VERSION}-64-linux.zip"
PHARO64_DIR_NAME="pharo${PHARO_VERSION}-64"

PHAROARM_PKG="latest.zip"
PHAROARM_DIR_NAME="pharo${PHARO_VERSION}"
...
#
APP_PKG_PATH=${PHARO_DIR_PATH}/${PHARO_PKG}
ARM_PKG_PATH=${PHARO_DIR_PATH}/${PHAROARM_PKG}
...
This script will install 32-bit Pharo properly on 64-bit Linux systems.  As
of v6.0, Pharo has been implemented in 64-bit; this script will install the
64-bit version if the platform is 64-bit.

To install Pharo from cached packages, rather than downloading from the
...
The '${PHARO_PKG}' file is in '${PHARO_DIR_PATH}', which you may keep
...
then
    ...
    cp "${SOURCE_DIR}"/"${PHARO_PKG}" "${PHARO_DIR_PATH}"/
	...
	if [[ ${IS_RASPBIAN_OS} ]]
	then
		cp "${SOURCE_DIR}"/"${PHAROARM_PKG}" "${PHARO_DIR_PATH}"/
...
#
echo
unzip "${APP_PKG_PATH}" -d "${PHARO_DIR_PATH}"

(( $? == 0 )) || ThrowError "${ERR_CMDFAIL}" "${APP_SCRIPT}" \
        "Could not unzip the '${PHARO_PKG}' package in '${PHARO_DIR_PATH}' !"
...
then
	...
	(( $? == 0 )) || ThrowError "${ERR_CMDFAIL}" "${APP_SCRIPT}" \
        "Could not unzip the '${PHAROARM_PKG}' pkg in '${PHARO_DIR_PATH}' !"

	move "${PHARO_DIR_PATH}"/"${PHAROARM_PKG}" \
		"${PHARO_DIR_PATH}"/pharo-linux-ARMv6.zip
fi
