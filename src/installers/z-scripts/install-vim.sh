...
#
SCRIPT_LIST="This script installs vim and gvim."

SCRIPT_GLOB="*.vim"
FindGlobFilename "basename" "${SOURCE_DIR}" 1 ${SCRIPT_GLOB}

if (( $? == 0 )); then
    SCRIPT_LIST="This script installs vim, gvim, and useful vim scripts:"
...
in a text user interface; its GUI mode, gVim, adds menus and toolbars for
commonly-used commands but the full functionality is still expressed through
